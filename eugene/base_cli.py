...
class AbstractCLI(ABC):
    """Abstract class for eugene command-line interface tools.

    Note:
        Tools are called from the command line using
        $ eugene TOOL-NAME --optional_arg1 optional_val1 ...
...
def generate_cli_dictionary() -> Dict[str, AbstractCLI]:
    ...
    for tool_name in TOOL_NAME_LIST:
        ...
        module_cli_str_list = ["eugene", tool_name.replace("-", "_"), "cli"]
...
def get_populated_argparser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="eugene",
        description="EUGENe is a Python toolkit for building and evaluating sequence-based deep learning models in genomics. "
        "It provides a unified workflow for managing data, training models, and interpreting predictions on biological sequences.",
    )
    ...
    subparsers = parser.add_subparsers(
        title="sub-commands", description="Valid eugene sub-commands, can be run with 'eugene {subcommand} [parameters]'", dest="tool"
    )
