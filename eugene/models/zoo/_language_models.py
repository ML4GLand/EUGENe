...
class SITh(nn.Module):
    ...
    def __init__(self, maxSeqLen: int, modelDim: int, mlpDim: int, depth: int, attnHeads: int, attnHeadDim: int, multitaskOutputs: list, trainingObjective: str, embeddingDropout: float = 0.0, dropout: float = 0.0, aggr: str = "context", vocabSize: int = 6, posEnc: str = "learned"):
        ...
        self.nukTuk = nn.Embedding(vocabSize, modelDim) #Project vocab index to modelDim. Why nukTuk? Because we have nucleotides (nuks) and we are tokenizing (tuk)/vectorizing them. Take down unalaq nuktuk!
                     
        self.context = nn.Parameter(torch.randn(1, 1, modelDim)) #'[CLS]' token representation
        
        if posEnc == "learned":
            logging.info("Utilizing FULLY-LEARNED positional encodings..")
            self.positionalEncoding = nn.Parameter(torch.randn(1, maxSeqLen + 1, modelDim))
        
        else: 
            positionalEncoding = FixedPositionalEncoding(maxSeqLen = maxSeqLen, modelDim = modelDim) #This is one dimension less - no context vector
            positionalEncoding = torch.cat([torch.zeros(1, modelDim), positionalEncoding], axis = 0) #Prepend 0 vector for context token - no sequence position
            
            if posEnc == "joint":
                logging.info("Utilizing LEARNED positional encodings on an initialized SIN-COS embedding")
                ...
                self.register_buffer("positionalEncoding", positionalEncoding.unsqueeze(0), persistent = False) #Ensures pushed to device; don't need saved as part of state_dict()
            
        self.embeddingDropout = nn.Dropout(embeddingDropout)
        self.preTransformerLayerNorm = nn.LayerNorm(modelDim)
        self.transformer = Megatron(modelDim = modelDim, blocks = depth, mlpDim = mlpDim, attnHeadDim = attnHeadDim, numHeads = attnHeads, dropout = dropout)
        self.aggr = aggr #how want to predict - on the context/CLS vector ("context") or mean pooling
        
        #CLS BERT-Style Hidden Layer
        self.clsHidden = nn.Sequential(nn.LayerNorm(modelDim), nn.Linear(modelDim, modelDim), nn.GELU()) #technically BERT uses nn.Tanh(), using gelu here instead
            
        if trainingObjective == "self-supervised":            
            #MLM Token level Hidden Layer
            self.jrrToken = nn.Sequential(nn.LayerNorm(modelDim), nn.Linear(modelDim, modelDim), nn.GELU(), nn.LayerNorm(modelDim))
        ...
        for mtlPredDim in multitaskOutputs:
            taskHead = nn.Sequential(nn.LayerNorm(modelDim), nn.Linear(modelDim, mtlPredDim))
            self.classificationHeads.append(taskHead)
        
    def forward(self, x, mask = None, **kwargs):  #**kwargs in case want to adapt for multiple inputs
        '''
        ...
        batchSize, n, _ = x.shape #n = max seq length --> Recall input sequences are padded to max seq length
        ...
        if mask is not None:
            ...
            mask = torch.cat([torch.tensor([False]).repeat(x.size(0), 1).to(mask.device), mask], dim = 1) #[b, n-1] --> [b,n]
