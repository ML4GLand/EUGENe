...
class PredictionWriter(BasePredictionWriter):
    ...
    def write_on_epoch_end(self, trainer, pl_module, outputs, batch_indices):
        ...
        pred_df = pd.DataFrame(data=outputs, columns=["NAMES"] + pred_cols + target_cols)
        pred_df.to_csv(os.path.join(self.output_dir, self.file_label) + "_predictions.tsv", sep="\t", index=False)
