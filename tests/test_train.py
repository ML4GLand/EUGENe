...
def test_fit_single_task(sdata):
    ...
    eu.train.fit(model, sdata, target_keys="activity_0", epochs=1, name="test_fit", version="singletask")
...
def test_fit_multitask(sdata):
    ...
    eu.settings.logging_dir = f"{HERE}/_logs"
    eu.train.fit(model, sdata, target_keys=[f"activity_{i}" for i in range(10)], epochs=1, name="test_fit", version="multitask")
