...
def test_boxplots(sdata):
    ax = eu.pl.boxplot(
        sdata,
        keys=["activity_0", "activity_1", "activity_2", "activity_3"],
        groupby="train_val",
        return_axes=True,
    )
    ax[0].set_xlabel("ETS Activity")
...
def test_performance_scatterplot(sdata):
    eu.pl.performance_scatter(
        sdata, target_keys="activity_1", prediction_keys="activity_1"
    )
...
def test_auroc(sdata):
    eu.pl.auroc(
        sdata,
        target_keys=["label_0", "label_0"],
        prediction_keys=["activity_0", "activity_1"],
    )
...
def test_seq_track_features(sdata):
    eu.pl.seq_track_features(
        sdata,
        seq_id=sdata.names[59],
        uns_key="InputXGradient_imps",
        pred_key="activity_0",
    )
...
def multifilter_viz(sdata):
    eu.pl.multifilter_viz(
        sdata, filter_ids=[0, 1], num_cols=2, num_rows=1, figsize=(10, 2)
    )
