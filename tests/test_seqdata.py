...
def test_init():
    names, seqs, rev_seqs, targets = eu.dl.read_numpy(f"{HERE}/_data/datasets/random1000/random_seqs.npy", names_file=f"{HERE}/_data/datasets/random1000/random_ids.npy", target_file=f"{HERE}/_data/datasets/random1000/random_labels.npy", rev_seq_file=f"{HERE}/_data/datasets/random1000/random_rev_seqs.npy", return_numpy=True)
    assert(len(names) == len(seqs) == len(rev_seqs) == len(targets))
    sdata = eu.dl.SeqData(names=names, seqs=seqs, seqs_annot=targets, rev_seqs=rev_seqs, pos_annot=f"{HERE}/_data/datasets/random1000/random_seq_features.bed")
...
def test_print(sdata):
    print(sdata)
    sdata.pos_annot = pr.read_bed(f"{HERE}/_data/datasets/random1000/random_seq_features.bed")
    print(sdata)

def test_write_h5sd(sdata):
    sdata.write_h5sd(f"{HERE}/_data/datasets/random1000/random_seqs.h5sd")
